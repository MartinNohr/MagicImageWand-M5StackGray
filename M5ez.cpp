...
// add a boolean handling version
bool ezMenu::addItem(String hdr, bool* pValue, char* trueString, char* falseString, bool (*fcn)(ezMenu*)) {
	// build the string
	hdr += String("\t") + (*pValue ? trueString : falseString);
	MenuItem_t new_item;
	new_item.nameAndCaption = hdr;
	new_item.advancedFunction = fcn;
	new_item.pBoolValue = pValue;
	new_item.trueString = trueString;
	new_item.falseString = falseString;
	if (_selected == -1)
...
// add an integer handling version
bool ezMenu::addItem(String hdr, int* pValue, int minval, int maxval, int decimals, bool (*fcn)(ezMenu*)) {
	// build the string
	hdr += "\t" + String(*pValue);
	MenuItem_t new_item;
	new_item.nameAndCaption = hdr;
	new_item.advancedFunction = fcn;
	new_item.pIntValue = pValue;
	new_item.minval = minval;
	new_item.maxval = maxval;
	new_item.decimals = decimals;
	if (_selected == -1)
		_selected = _items.size();
	_items.push_back(new_item);
	_sortItems();
