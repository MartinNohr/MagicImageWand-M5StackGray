...
class ezMenu {
	public:
		...
		bool addItem(String hdr, int* pValue, int minval, int maxval, int decimals, bool (*fcn)(ezMenu*));
		bool addItem(String hdr, bool* pValue, char* trueString, char* falseString, bool (*fcn)(ezMenu*));
		...
		// get the item name from ix without any \t in front of the variable
		String getItemName(int ix) {
			if (ix > 0 && ix <= _items.size()) {
				// remove the \t if it was there
				String str = _items[ix - 1].nameAndCaption;
				int ix = str.lastIndexOf('\t');
				if (ix != -1)
					str = str.substring(0, ix - 1);
				return str;
			}
			return "";
		}
		String getItemName() {
			if (_selected != -1) {
				// remove the \t if it was there
				String str = _items[_selected].nameAndCaption;
				int ix = str.lastIndexOf('\t');
				if (ix != -1)
		...
		int* getIntValue() {
			if (_selected != -1) {
				return _items[_selected].pIntValue;
			}
			return NULL;
		}
		int getIntMinVal() {
			if (_selected != -1) {
				return _items[_selected].minval;
			}
			return 0;
		}
		int getIntMaxVal() {
			if (_selected != -1) {
				return _items[_selected].maxval;
			}
			return 0;
		}
		int getIntDecimals() {
			if (_selected != -1) {
				return _items[_selected].decimals;
			}
			return 0;
		}
		bool* getBoolValue() {
			if (_selected != -1) {
				return _items[_selected].pBoolValue;
			}
			return 0;
		}
		char* getBoolTrue() {
			if (_selected != -1) {
				return _items[_selected].trueString;
			}
			return "";
		}
		char* getBoolFalse() {
			if (_selected != -1) {
				return _items[_selected].falseString;
			}
			return "";
		}
	...
	private:
		...
		struct MenuItem_t {
			...
			// int values for editing
			int* pIntValue;
			int minval;
			int maxval;
			int decimals;
			// bool values for toggling
			bool* pBoolValue;
			char* trueString;
			char* falseString;
		};
